...
public class TresEnRaya {
    ...
    public int minMaxHum(Tablero datos, int jugador, int nivel, boolean bandera)
    {
        if (finDelJuego(datos) && jugador == MIN) {
            return funcionDeEvaluacion(datos, jugador);
        } else if (finDelJuego(datos) && jugador == MAX) {
        ...
        while (!listaReglas.isEmpty()) {
            ...
            if (jugador == MAX) {
                valor = mayor(valor, minMaxHum(nuevoTablero, MIN, nivel - 1, false));
                if (aux != valor && bandera) {
                    puntoJugador = p;
                }
            } else {
                valor = menor(valor, minMaxHum(nuevoTablero, MAX, nivel - 1, false));
                if (aux != valor && bandera) {
                    puntoJugador = p;
                }
            }
